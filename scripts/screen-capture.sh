...
# Detect available displays for VM compatibility
get_primary_display() {
  # Try to get the primary display from Hyprland
  primary_display=$(hyprctl monitors -j | jq -r '.[0].name' 2>/dev/null)
  
  # If that fails or returns empty, try wlr-randr
  if [ -z "$primary_display" ] || [ "$primary_display" = "null" ]; then
    primary_display=$(wlr-randr 2>/dev/null | grep -m1 "^[^ ]" | awk '{print $1}')
  fi
  
  # Default to XWAYLAND0 for VMs if still empty
  if [ -z "$primary_display" ]; then
    primary_display="XWAYLAND0"
  fi
...
Examples:
  ...
  $SCRIPT_NAME screenshot display
  $SCRIPT_NAME record display
  $SCRIPT_NAME record-noaudio selection
  $SCRIPT_NAME record-hq display
...
"screenshot")
	...
	"selection")
		...
		;;
	"display")
		grim -c -o "$PRIMARY_DISPLAY" "$IMG"
		wl-copy <"$IMG"
		send_screenshot_notification "$IMG"
		;;
	"all")
		# Get all displays
		displays=$(hyprctl monitors -j | jq -r '.[].name' 2>/dev/null)
		if [ -z "$displays" ]; then
			# Fallback to just capturing everything
			grim "$IMG"
			wl-copy <"$IMG"
			send_screenshot_notification "$IMG"
			return
...
"record")
	...
	"display")
		...
		echo "$VID" >/tmp/recording.txt
		date +%s >/tmp/recording_start_time.txt
		record_video "$VID" -a -o "$PRIMARY_DISPLAY"
		;;
	# Legacy options for backward compatibility
	"eDP-1")
		wf-recorder_check
...
"record-noaudio")
	...
	"display")
		...
		echo "$VID" >/tmp/recording.txt
		date +%s >/tmp/recording_start_time.txt
		record_video_noaudio "$VID" -o "$PRIMARY_DISPLAY"
		;;
	# Legacy options for backward compatibility
	"eDP-1")
		wf-recorder_check
...
"record-hq")
	...
	"display")
		...
		echo "$VID_HQ" >/tmp/recording.txt
		date +%s >/tmp/recording_start_time.txt
		notify-send "High Quality Recording" "Starting YouTube-quality recording on $PRIMARY_DISPLAY..."
		record_high_quality "$VID_HQ" -a -o "$PRIMARY_DISPLAY"
		;;
	# Legacy options for backward compatibility
	"eDP-1")
		wf-recorder_check
...
"record-gif")
	...
	"display")
		...
		echo "$GIF" >/tmp/recording.txt
		date +%s >/tmp/recording_start_time.txt
		notify-send "GIF Recording" "Starting GIF recording on $PRIMARY_DISPLAY..."
		record_gif "$GIF" -o "$PRIMARY_DISPLAY"
		;;
	# Legacy options for backward compatibility
	"eDP-1")
		wf-recorder_check
		echo "$GIF" >/tmp/recording.txt
		date +%s >/tmp/recording_start_time.txt
		notify-send "GIF Recording" "Starting GIF recording..."
		record_gif "$GIF" -o "$PRIMARY_DISPLAY"
		;;
	"HDMI-A-1")
		wf-recorder_check
